...
namespace lingvo.ts.modelbuilder
{
    ...
    internal sealed class Portioner
    {
        ...
        {
            public static tuple_tf_term_Comparer Inst { get; } = new tuple_tf_term_Comparer();
        ...
        {
            public static tuple_tf_frequency_Comparer Inst { get; } = new tuple_tf_frequency_Comparer();
        ...
        {
            public static stringComparer Inst { get; } = new stringComparer();
        ...
        #region [.ctor().]
        public Portioner( in BuildParams_t bp, FileInfo fi, TFProcessor tfProcessor )
        {
            _Bp = bp;
            _Fi = fi;
            ...
            _DocumentNgrams_1 = new Dictionary< string, int >( _Bp.MaxPortionSize, stringComparer.Inst );

            switch ( _Bp.Ngrams )
            {
                case NGramsEnum.ngram_4:
                    _OutputFilenames.Add( NGramsEnum.ngram_4, new List< string >() );
                    _DocumentNgrams_4 = new Dictionary< string, int >( _Bp.MaxPortionSize, stringComparer.Inst );
                goto case NGramsEnum.ngram_3;

                case NGramsEnum.ngram_3:
                    _OutputFilenames.Add( NGramsEnum.ngram_3, new List< string >() );
                    _DocumentNgrams_3 = new Dictionary< string, int >( _Bp.MaxPortionSize, stringComparer.Inst );
                goto case NGramsEnum.ngram_2;

                case NGramsEnum.ngram_2:
                    _OutputFilenames.Add( NGramsEnum.ngram_2, new List< string >() );
                    _DocumentNgrams_2 = new Dictionary< string, int >( _Bp.MaxPortionSize, stringComparer.Inst );
                break;
            }
        ...
        private static IEnumerable< TermFrequency > GroupByMerging_1    ( List< string > fileNames )
        {
            ...
            {
                current_tuples.Sort( tuple_tf_term_Comparer.Inst );
        ...
        private static IEnumerable< TermFrequency > GroupByMerging_2    ( List< tuple >  tuples )
        {
            for ( ; tuples.Count != 0; )
            {
                tuples.Sort( tuple_tf_frequency_Comparer.Inst );
        ...
        private static string Write2File( FileInfo fi, int portionNumber, Dictionary< string, int > tf_matrix, NGramsEnum tf_matrix_type, int currentProcesId )
        {
            ...
            var ss = new SortedDictionary< string, int >( stringComparer.Inst );
    ...
    internal static class ModelBuilder
    {
        ...
        public static void Build( in BuildParams_t bp, int tfProcessorDictionaryCapacity = TFPROCESSOR_DICTIONARY_CAPACITY )
        {
            #region [.-0-.]
            ...
            var skipWordCount = 0;
#endif
            var singleWordMaxLength = bp.SingleWordMaxLength;
            var processWordAction = default(Action< string >);
            if ( bp.ClearCyrillicsChars )
            {
                if ( bp.ClearDigitsChars )
                {
                    processWordAction = (word) =>
                    {
                        if ( !word.HasCyrillicsOrDigitsChars() )
                        {
                            if ( word.Length <= singleWordMaxLength )
                            {
        ...
        public static void Build_UsePortion( in BuildParams_t bp, int tfProcessorDictionaryCapacity = TFPROCESSOR_DICTIONARY_CAPACITY )
        {
            #region [.-0-.]
            Console.WriteLine( $"start process folder: '{bp.InputDirectory}'..." );
        ...
        private static void BuildTFMatrix_UsePortion( in BuildParams_t bp, FileInfo fi, TFProcessor tfProcessor )
        {
